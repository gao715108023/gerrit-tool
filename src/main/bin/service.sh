...
#这里写自己服务的名字
SERVICE_NAME="gerrit-code-review-service"
...
JAVA_OPT_1="-server -Xms1g -Xmx1g -Xmn512m"
JAVA_OPT_2="-XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=70 -XX:+CMSParallelRemarkEnabled -XX:SoftRefLRUPolicyMSPerMB=0 -XX:+CMSClassUnloadingEnabled -XX:SurvivorRatio=8 -XX:+DisableExplicitGC"
...
function startService {
   ...
   while [ $COUNT -lt 1 ]; do
    for (( i=0;  i<60;  i=i+1 )) do
        STR=`grep "gerrit code review service server started" nohup.log`
        if [ ! -z "$STR" ]; then
            echo "PID=$pid"
